...
from .models import CareIndicator, CareAction, VirtualWallet
...
def gamification_dashboard(request, animal_id):
    ...
    recent_actions = CareAction.objects.filter(
...
def feed_animal(request, animal_id):
    ...
    if care_indicator.food_level >= 100:
        return JsonResponse({
            'success': False,
            'error': 'El nivel de alimento ya está al máximo'
        }, status=400)
...
def clean_animal(request, animal_id):
    ...
    if care_indicator.hygiene_level >= 100:
        return JsonResponse({
            'success': False,
            'error': 'El nivel de higiene ya está al máximo'
        }, status=400)
